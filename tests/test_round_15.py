...
def test_trend_sl_tp_exits_routed_through_confirm_trade_exit():
    ...
    from combo_bot.types import (
        Candle, Order, OrderSource, Position, Side, SymbolState,
    )
    ...
    trend_exits = bt.merger.generate_trend_exit_orders(
        "BTC", ss.trend_long, Side.LONG, sl_price, __import__("combo_bot.types", fromlist=["ExchangeParams"]).ExchangeParams(),
    )
    ...
    acc.symbols["BTC"] = ss
    ctx = TradeContext(
        symbol="BTC", side=Side.LONG, position=ss.trend_long,
        account=acc,
    ...
    # Vetoed → empty.
    assert survived == []
    assert _VetoTrendExitStrategy.saw_calls, (
...
def test_confirm_trade_exit_receives_final_price_after_adjustments():
    ...
    from combo_bot.types import (
        ...
        Side, SymbolState, TrendSignal, TrendRegime,
    )
    ...
    acc.symbols["BTC"] = ss
    ctx = TradeContext(
        symbol="BTC", side=Side.LONG, position=pos, account=acc,
        candle=Candle(0, 50_500, 50_500, 50_500, 50_500, 0),
        signal=TrendSignal(direction=0, strength=0, regime=TrendRegime.NEUTRAL),
        current_time_ms=0, exchange_params=ExchangeParams(),
    )
    exit_order = Order(
        symbol="BTC", side=Side.LONG, price=51_000.0, qty=0.01,
...
def test_fill_polling_drains_until_short_page():
    ...
    class _Ex:
        ...
        async def fetch_my_trades(self, symbol, since=None, limit=None):
            ...
            if since is None:
                return [
                    {"id": "t1", "timestamp": 1_000, "side": "buy",
                     "price": 50_000.0, "amount": 0.001, "fee": {"cost": 0.0},
                     "order": "ord-1", "info": {}},
                    {"id": "t2", "timestamp": 1_000, "side": "buy",
                     "price": 50_000.0, "amount": 0.001, "fee": {"cost": 0.0},
                     "order": "ord-1", "info": {}},
                ]
            if since == 1_000:
                return [
                    {"id": "t3", "timestamp": 1_500, "side": "buy",
                     "price": 50_000.0, "amount": 0.001, "fee": {"cost": 0.0},
                     "order": "ord-1", "info": {}},
                    {"id": "t4", "timestamp": 1_500, "side": "buy",
                     "price": 50_000.0, "amount": 0.001, "fee": {"cost": 0.0},
                     "order": "ord-1", "info": {}},
                ]
            if since == 1_500:
                return [
                    {"id": "t5", "timestamp": 2_000, "side": "buy",
                     "price": 50_000.0, "amount": 0.001, "fee": {"cost": 0.0},
                     "order": "ord-1", "info": {}},
                ]
            return []

    mgr = FillEventManager(_Ex(), FillEventManagerConfig(
        poll_interval_ms=0, page_size=2,
    ))
    captured: list[Fill] = []
    asyncio.run(mgr.poll("BTC/USDT:USDT", now_ms=3_000, sink=captured.extend))
    captured_ids = {f.trade_id for f in captured}
    assert captured_ids == {"t1", "t2", "t3", "t4", "t5"}, (
...
def test_fill_polling_breaks_when_cursor_not_advancing():
    ...
    class _StuckEx:
        ...
        async def fetch_my_trades(self, *_a, **_k):
            self.calls += 1
            return [
                {"id": f"t-{self.calls}-a", "timestamp": 1_000, "side": "buy",
                 "price": 50.0, "amount": 0.001, "fee": {"cost": 0.0},
                 "order": "ord-1", "info": {}},
                {"id": f"t-{self.calls}-b", "timestamp": 1_000, "side": "buy",
                 "price": 50.0, "amount": 0.001, "fee": {"cost": 0.0},
                 "order": "ord-1", "info": {}},
            ]

    ex = _StuckEx()
    mgr = FillEventManager(ex, FillEventManagerConfig(
        poll_interval_ms=0, page_size=2,
    ))
    captured = []
