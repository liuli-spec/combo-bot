...
from combo_bot.types import ExchangeParams
...
def make_osc(n: int, base: float, amp: float, phase: float = 0.0) -> np.ndarray:
    ...
    spread = amp * 0.05
    return np.column_stack([close, close + spread, close - spread, close, np.full(n, 100.0)])
...
class TestMultiSymbolBasic:
    def test_runs_three_symbols(self):
        ...
        cfgs = {s: GridConfig(wallet_exposure_limit=0.3, ema_span_0=30, ema_span_1=60) for s in data}
...
class TestForagerSelection:
    def test_n_positions_limits_active_symbols(self):
        ...
        active_at_end = sum(
            1 for p in result.final_positions
...
class TestMultiSymbolMetrics:
    def test_shared_balance_drawdown(self):
        ...
        cfgs = {"X": GridConfig(wallet_exposure_limit=0.5, ema_span_0=30, ema_span_1=60)}
        result = multi.run_multi_symbol_backtest(
            data, cfgs,
            bt_config=multi.MultiSymbolConfig(n_positions_max=1, starting_balance=10000),
        )
