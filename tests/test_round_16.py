...
class _Stub:
    ...
    async def fetch_positions(self, _): return self.positions
...
def test_fetch_order_resolved_bucket_not_double_applied_by_late_trade():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        trader.intent_journal.submit(
            cid="cb-target", symbol="BTC/USDT:USDT", side="long",
            source="trend", reduce_only=False, is_market=True, now_ms=1,
        )
        trader.intent_journal.open(
            cid="cb-target", exchange_id="ex-7", now_ms=2,
        )
        ...
        trader.exchange.trades_by_call = [[
            {
                "id": "t-1", "timestamp": 9_000, "side": "buy",
                "price": 50_000.0, "amount": 0.01, "fee": {"cost": 0.05},
                "order": "ex-7", "clientOrderId": "cb-target",
                "info": {"realizedPnl": "0"},
            },
        ]]
    ...
    class _BurstEx:
        ...
        async def fetch_my_trades(self, symbol, since=None, limit=None, params=None):
            ...
            if from_id is None:
                ...
                ids = []
            return [
                {"id": tid, "timestamp": burst_ts, "side": "buy",
...
def test_custom_exit_can_be_vetoed_by_confirm_trade_exit():
    ...
    bt = Backtester(BacktestConfig(starting_balance=10_000), strategy=_StratWithExitAndVeto())
    ...
    risk_closes = [
        ...
        if f.source.value == "risk" and f.reduce_only
    ]
    assert risk_closes == [], (
...
def test_market_exit_skips_custom_exit_price():
    ...
    acc = AccountState(balance=10_000); acc.symbols["BTC"] = ss
    ctx = TradeContext(
    ...
    market_exit = Order(
        symbol="BTC", side=Side.LONG, price=50_000.0, qty=0.01,
        source=OrderSource.GRID, reduce_only=True, is_market=True,
    )
