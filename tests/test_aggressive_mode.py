...
class TestGridAggressiveMode:
    def test_aggressive_stacks_more_qty_than_normal(
    ...
    def test_aggressive_spacing_pulls_entries_closer(
        self, ema_state, volatility_state, exchange_params
    ):
        engine = GridEngine(
            GridConfig(
...
class TestCloseMarkupMultiplier:
    def test_multiplier_compresses_close_prices(
        self, ema_state, volatility_state, exchange_params
    ):
        engine = GridEngine(
            GridConfig(
...
class TestBacktesterRegimeWiring:
    def test_default_arbiter_does_not_change_oscillating_market_result_drastically(
        self,
    ):
