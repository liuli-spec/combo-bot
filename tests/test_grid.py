...
from combo_bot.grid_engine import GridConfig, GridEngine, quantize_qty, quantize_price, calc_wallet_exposure
...
class TestGridEntries:
    ...
    def test_generates_entries_when_no_position(self, engine, ema_state, volatility_state, exchange_params):
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, Position(),
            ema_state, volatility_state, 10000.0, 0.0,
    ...
    def test_entry_prices_decrease_for_long(self, engine, ema_state, volatility_state, exchange_params):
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, Position(),
            ema_state, volatility_state, 10000.0, 0.0,
            exchange_params, TradingMode.NORMAL,
    ...
    def test_entry_prices_increase_for_short(self, engine, ema_state, volatility_state, exchange_params):
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.SHORT, Position(),
            ema_state, volatility_state, 10000.0, 0.0,
            exchange_params, TradingMode.NORMAL,
        )
        entries = [o for o in orders if not o.reduce_only]
    ...
    def test_qty_increases_with_double_down(self, engine, ema_state, volatility_state, exchange_params):
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, Position(),
            ema_state, volatility_state, 10000.0, 0.0,
            exchange_params, TradingMode.NORMAL,
    ...
    def test_respects_wallet_exposure_limit(self, engine, ema_state, volatility_state, exchange_params):
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, Position(),
            ema_state, volatility_state, 10000.0, 0.0,
            exchange_params, TradingMode.NORMAL,
    ...
    def test_no_entries_in_tp_only_mode(self, engine, ema_state, volatility_state, exchange_params):
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, Position(),
            ema_state, volatility_state, 10000.0, 0.0,
            exchange_params, TradingMode.TP_ONLY,
        )
        entries = [o for o in orders if not o.reduce_only]
        assert len(entries) == 0
...
class TestGridCloses:
    ...
    def test_generates_closes_for_open_long(self, engine, ema_state, volatility_state, exchange_params):
        pos = Position(size=0.1, entry_price=50000.0)
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, pos,
            ema_state, volatility_state, 10000.0, 0.5,
            exchange_params, TradingMode.NORMAL,
        )
    ...
    def test_generates_closes_with_position_side_for_open_short(self, engine, ema_state, volatility_state, exchange_params):
        pos = Position(size=0.1, entry_price=50000.0)
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.SHORT, pos,
            ema_state, volatility_state, 10000.0, 0.5,
            exchange_params, TradingMode.NORMAL,
        )
        closes = [o for o in orders if o.reduce_only]
    ...
    def test_close_prices_above_entry_for_long(self, engine, ema_state, volatility_state, exchange_params):
        pos = Position(size=0.1, entry_price=50000.0)
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, pos,
            ema_state, volatility_state, 10000.0, 0.5,
    ...
    def test_close_prices_below_entry_for_short(self, engine, ema_state, volatility_state, exchange_params):
        pos = Position(size=0.1, entry_price=50000.0)
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.SHORT, pos,
            ema_state, volatility_state, 10000.0, 0.5,
    ...
    def test_close_qty_sums_to_position_size(self, engine, ema_state, volatility_state, exchange_params):
        pos = Position(size=0.1, entry_price=50000.0)
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, pos,
            ema_state, volatility_state, 10000.0, 0.5,
    ...
    def test_panic_mode_closes_full_position(self, engine, ema_state, volatility_state, exchange_params):
        pos = Position(size=0.1, entry_price=50000.0)
        orders = engine.compute_orders(
            "BTC/USDT:USDT", Side.LONG, pos,
            ema_state, volatility_state, 10000.0, 0.5,
            exchange_params, TradingMode.PANIC,
        )
        assert len(orders) == 1
